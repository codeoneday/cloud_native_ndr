...
void dpi_tcp_tracker(dpi_packet_t *p)
{
    ...
    if (unlikely(s == NULL)) {
        if (tcph->syn) {
            ...
            if (!tcph->ack) {
                ...
                tcp_open_by_syn(s);//设置上行tcp和下行tcp的状态
        ...
        dpi_session_timer_refresh(s);//更新会话时间轮中节点
    }
    ...
    if (len > 0) {
        // If parsers are ignored, no one moves the asm_seq, no point to
        // assemble or sequence the session.
        // 如果解析器被忽略，则没有人移动 asm_seq，没有必要组装或排序会话。
